...
print 'When the programme encounters an error, or takes too long - Rerun'
with session() as c:
...
def spin(key):
	threads = [Arachne(target=fetch_url, args=(code,key)) for code in Codedict[key]]
...
def gener(dn):
	for count in xrange (len(time_index)):
		if count==0:
			yield [time_index[count]]+[a[count] for a in  dn]
		else:
			yield [time_index[count]]+[a[count][:a[count].index(',')] for a in  dn]
